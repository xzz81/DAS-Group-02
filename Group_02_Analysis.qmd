...
author: "Group02: Yue Xing, Chengyang Dong, Jingyi Zhu, Qinyuan Ye"
...
Considering that "Total.Number.of.Family.members" is a set of counting variables, consisting of discrete non-negative integers. Therefore, it was chosen to first observe the characteristics of the distribution of the dependent variable.

```{r, fig.cap="Dependent variable data distribution"}
# View outcome variables
ggplot(data, aes(Total.Number.of.Family.members)) +
  geom_histogram()
```

The Figure 1 shows that the distribution of the "Total.Number.of.Family.members" variable is right skewed. Combined with the fact that the variable is a set of count-type variables, it can be roughly tentatively assumed that the total number of family members shows a Poisson distribution.
...
```{r}
#| tab-cap: "Descriptive statistical analysis of selected variables"
...
```{r,fig.width=6.5, fig.cap="Correlation graph except categorical variables"}
...
From the correlation graph of Figure 2 we can see three variables most correlate with the total number of family member: 1.Total household income 2.Total food expenditure 3.Household head age.

```{r, fig.width=6.5, fig.cap="The pairs plot of strong correlate variable"}
...
Then we draw boxplots of these variables to analyse:
...
```{r, fig.width=6.5, fig.cap="The boxplot of total numbers of family members and gender of head of household"}
...
```{r, fig.width=6.5, fig.cap="The boxplot of total numbers of family members and type of household"}
...
```{r, fig.width=6.5, fig.cap="The boxplot of total numbers of family members and electricity"}
...
As can be seen from the Figure 4 to Figure 6, there are significant differences between sex of household head and type of household, but the distribution of the total of number of family members is basically the same under different electricity types. Therefore, it is inferred that this variable has no significant impact on the number of family members.

- Second, we are also interested in whether food expenditures differed by type of household.

```{r, fig.width=6.5, fig.cap="The boxplot of total food expenditure and type of household"}
...
Through the Figure 7, it is found that the difference in food expenditures among different house types is smaller than the difference in number of family members.

```{r, fig.width=6.5, fig.cap="The boxplot of number of family member, food expenditure and house area"}
p1 <- ggplot(data = data, aes(x = "",y = Total.Household.Income)) +
  geom_boxplot() +
  labs(x = "Total.Household.Income", y = "")
p2 <- ggplot(data = data, aes(x = "",y = Total.Food.Expenditure)) +
  geom_boxplot() +
  labs(x = "Total.Food.Expenditure", y = "")
p3 <- ggplot(data = data, aes(x = "",y = House.Floor.Area)) +
  geom_boxplot() +
  labs(x = "House.Floor.Area", y = "")
p1+p2+p3+plot_layout(nrow=1)
```

Figure 8 is a box plot of number of family member, food expenditure and house area. The data distribution of these variables is analyzed through the box plot.
...
First, fit all variables into a generalized linear regression model.
...
All variables are significant in Model 4.
...
From both AIC and cross validation result, the model3 is the best model. It has lower AIC and lower residual_deviance.
...
```{r}
dispersion_index <- sum(residuals(poisson_model_3, type = "pearson")^2) / df.residual(poisson_model_3)
...
```{r, fig.width=6.5, fig.cap="Residuals plot of Model 3"}
# Residuals plot of Model 3
...
```{r, fig.width=10, fig.height=8,fig.cap="Residuals for different variables"}
...
```{r,fig.width=6.5, fig.cap="Q-Q plot of Model 3"}
qqnorm(residuals)
