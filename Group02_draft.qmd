...
Compare it with the old  model.
```{r}
par(mfcol=c(1,2))
plot(residuals ~ fitted.values(poisson_model_4),
     xlab = "Fitted Values",
     ylab = "Residuals",
     main = "Residual Plot for new model",
     cex=1,
     pch=20)+
abline(h=0, lty=2, col="red",lwd=3)
...
plot(residuals(poisson_model_1) ~ fitted.values(poisson_model_1),
     xlab = "Fitted Values",
     ylab = "Residuals",
     main = "Residual Plotfor original model",
     cex=1,
     pch=20)+
abline(h=0, lty=2, col="red",lwd=3)
```
...
1.Residual Plot for total household income
```{r}
...
2.Residual Plot for total food expenditure
```{r}
...
3.Residual Plot for household head sex
```{r}
...
4.Residual Plot for household head age
```{r}
...
5.Residual Plot for type of household
```{r}
